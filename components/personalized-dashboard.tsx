...
export default function PersonalizedDashboard({
  ...
  const calculateUserStats = () => {
    // Calculate total trips based on historical data length
    const totalTrips = historicalData.length

    // Calculate average travel time (simulated, as actual route data isn't stored per trip)
    const avgTravelTime =
      totalTrips > 0
        ? Math.round(historicalData.reduce((sum, d) => sum + (d.avgSpeed > 0 ? 30 : 45), 0) / totalTrips)
        : 0

    // Simulate fuel saved and CO2 reduced based on some random factor or historical data
    const fuelSaved = Math.round(Math.random() * 50 + 20)
    const co2Reduced = Math.round(Math.random() * 100 + 50)

    // Simulate favorite routes and peak travel times
    const favoriteRoutes = ["Highway 101", "Main Street", "Oak Avenue"]
    const peakTravelTimes = ["8:00 AM", "5:30 PM"]
    const preferredDays = ["Monday", "Wednesday", "Friday"]

    // Simulate efficiency based on current traffic data or a random value
    const efficiency =
      trafficData.length > 0
        ? Math.round((1 - trafficData.reduce((sum, p) => sum + p.congestion, 0) / trafficData.length) * 100)
  ...
  const achievements = [
    ...
    { id: 2, title: "Route Master", description: "Used 10+ different routes", icon: "🗺️", unlocked: true },
    {
      ...
      icon: "🌱",
      unlocked: (userStats?.co2Reduced || 0) > 50,
    },
    { id: 4, title: "Community Helper", description: "Submitted 5+ traffic reports", icon: "🤝", unlocked: false }, // This would need actual social report count
    {
      id: 5,
      title: "Speed Demon",
      description: "Average speed > 60 km/h",
      icon: "⚡",
      unlocked: (userStats?.avgSpeed || 0) > 60,
    }, // Needs avgSpeed from userStats
    { id: 6, title: "Night Owl", description: "Travel after 10 PM", icon: "🦉", unlocked: false }, // Needs actual travel time data
  ]
