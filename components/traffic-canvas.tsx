...
export default function TrafficCanvas({
  ...
  const drawTrafficView = (ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    if (filters) {
      filteredData = trafficData.filter((point) => {
        ...
        // Incidents filter
        if (filters.showIncidents === false && point.incidents) return false
        // Construction filter (assuming incidents can also be 'construction')
        if (filters.showConstruction === false && point.incidents === "construction") return false
  ...
  const drawHeatmapView = (ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    // Calculate min/max congestion for scaling
    const minCongestion = trafficData.reduce((min, p) => Math.min(min, p.congestion), 1)
    const maxCongestion = trafficData.reduce((max, p) => Math.max(max, p.congestion), 0)
    const congestionRange = maxCongestion - minCongestion

    for (let x = 0; x < width; x += 2) {
      for (let y = 0; y < height; y += 2) {
        const intensity = calculateHeatmapIntensity(x, y, width, height, minCongestion, congestionRange)
  ...
  const drawPredictionsView = (ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    predictions.forEach((prediction, index) => {
      // Map prediction to a spatial location for visualization
      // For simplicity, let's distribute them around the user's location or center
      const baseLat = userLocation?.lat || 0
      const baseLng = userLocation?.lng || 0

      const angle = (index / predictions.length) * 2 * Math.PI
      const distance = 0.05 // Arbitrary distance for visual spread
      const predLat = baseLat + Math.cos(angle) * distance
      const predLng = baseLng + Math.sin(angle) * distance

      const x = width / 2 + (predLng - (userLocation?.lng || 0)) * 8000 * zoom
      const y = height / 2 - (predLat - (userLocation?.lat || 0)) * 8000 * zoom
  ...
  const drawEnhancedGrid = (ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    for (let y = offsetY; y <= height; y += gridSize) {
      ctx.beginPath()
      ctx.moveTo(0, y)
      ctx.lineTo(0, y) // Fix: Should be ctx.lineTo(width, y)
      ctx.stroke()
    }
  }
  ...
  const calculateHeatmapIntensity = (
    ...
    trafficData.forEach((point) => {
      ...
      const influence = Math.max(0, 1 - distance / 50) // Influence radius of 50 pixels

      // Normalize congestion to 0-1 range based on min/max in current data
      const normalizedCongestion = congestionRange > 0 ? (point.congestion - minCongestion) / congestionRange : 0

      intensity += normalizedCongestion * influence
    })
  ...
  const getHeatmapColor = (intensity: number) => {
    ...
    // From green (low) to yellow (medium) to red (high)
    const colors = [
    ...
    // Scale intensity to the number of color segments
    ...
    // Handle edge cases for intensity
  ...
  const drawTrafficFlow = (ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    trafficData.forEach((point, index) => {
      ...
      if (x >= 0 && x <= width && y >= 0 && y <= height) {
        // Simulate direction based on speed or a random angle
        const angle = (time + index) % (2 * Math.PI)
        const arrowLength = 15 * (point.speed / 80) // Arrow length scales with speed
  ...
  const drawEnhancedNetworkIndicator = (
    ...
    for (let i = 0; i < bars; i++) {
      ...
      const barY = y + 25 - (maxHeight / bars) * (i + 1) // Calculate bar height from bottom up
      ...
      ctx.fillRect(barX, barY, barWidth, (maxHeight / bars) * (i + 1)) // Draw bar from bottom up
    }
  }
