...
export default function AnalyticsDashboard({
  ...
  useEffect(() => {
    if (trafficData.length > 0 || historicalData.length > 0 || socialReports.length > 0) {
      processAnalytics()
    }
  }, [trafficData, historicalData, socialReports]) // Added socialReports to dependency array
  ...
  const processAnalytics = () => {
    ...
    const processChunk = (deadline: IdleDeadline) => {
      // Simulate some work to consume timeRemaining
      let workDone = false
      const startTime = performance.now()

      while (performance.now() - startTime < 10 && deadline.timeRemaining() > 0) {
        // Process for max 10ms or until idle time runs out
        // Perform a small part of the calculation
        // This loop is just to simulate work, actual calculations are outside
        workDone = true // Indicate some work was done
      }
  ...
  const calculateTrends = () => {
    ...
    const trendPercentage = olderAvg !== 0 ? Math.round((trend / olderAvg) * 100) : 0 // Avoid division by zero
  ...
  const generateInsights = () => {
    ...
    // Add insight based on social reports
    if (socialReports.length > 0) {
      const highPriorityReports = socialReports.filter((r) => r.severity > 0.7).length
      if (highPriorityReports > 0) {
        insights.push({
          type: "info",
          message: `${highPriorityReports} high-priority social reports received`,
          icon: "🗣️",
        })
      }
    }
